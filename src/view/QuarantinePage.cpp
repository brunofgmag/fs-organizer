    ...
    : QWidget(parent), viewModel_(viewModel), model_(model)
{
    ...
    panel_->Content()->insertWidget(1, restoreFromPanel_);
    panel_->Content()->insertWidget(2, openFolder_);
    panel_->Content()->insertWidget(3, promise);
...
void QuarantinePage::ShowTheSelectedBatch(const QModelIndexList& rows)
{
    int known = 0;
