...
void SetupService::Detect()
{
    candidates_ = locator_.Locate();
}

const std::vector<SimulatorCandidate>& SetupService::Candidates() const
{
    return candidates_;
}

DestinationCheck SetupService::CheckDestination(const std::filesystem::path& path) const
{
...
void SetupService::AddManualCandidate(const std::filesystem::path& destination, const SimulatorVariant variant)
{
...
void SetupService::ChooseCandidate(const std::size_t index)
{
    chosen_ = index;
}

LibraryReport SetupService::RegisterLibrary(const std::filesystem::path& path, const std::string& label)
{
