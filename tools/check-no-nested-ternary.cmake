...
set(OPENS_A_GROUP "O")
set(CLOSES_A_GROUP "C")
set(SEPARATES_ARGUMENTS "S")
set(ENDS_THE_STATEMENT "E")
set(ASKS "Q")
...
foreach (DIRECTORY IN LISTS SEARCHED_DIRECTORIES)
    ...
    foreach (SOURCE IN LISTS SOURCES)
        ...
        foreach (LINE IN LISTS LINES)
            ...
            string(REGEX REPLACE "[([{]" "${OPENS_A_GROUP}" SKELETON "${SKELETON}")
            string(REGEX REPLACE "[])}]" "${CLOSES_A_GROUP}" SKELETON "${SKELETON}")
            string(REPLACE "," "${SEPARATES_ARGUMENTS}" SKELETON "${SKELETON}")
            string(REPLACE ";" "${ENDS_THE_STATEMENT}" SKELETON "${SKELETON}")
            string(REPLACE "?" "${ASKS}" SKELETON "${SKELETON}")
            ...
            foreach (POSITION RANGE ${LAST})
                ...
                if (MARK STREQUAL "${OPENS_A_GROUP}")
                    math(EXPR DEPTH "${DEPTH} + 1")
                elseif (MARK STREQUAL "${CLOSES_A_GROUP}")
                    math(EXPR DEPTH "${DEPTH} - 1")
                ...
                elseif (MARK STREQUAL "${SEPARATES_ARGUMENTS}")
                    list(POP_BACK OPEN_TERNARIES INNERMOST)
                ...
                elseif (MARK STREQUAL "${ENDS_THE_STATEMENT}")
                    set(OPEN_TERNARIES "")
                elseif (MARK STREQUAL "${ASKS}")
                    if (OPEN_TERNARIES)
